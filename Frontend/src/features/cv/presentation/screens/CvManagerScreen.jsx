        ...
        <div className="relative z-10 flex flex-col items-center justify-between gap-8 md:flex-row">
          <div className="max-w-2xl text-center md:text-left space-y-4">
             ...
             <p className="text-sm font-medium text-slate-500 leading-relaxed max-w-xl">
                Upload your CV and let our AI extract your skills and help you match with jobs.
             </p>
             <div className="flex flex-wrap gap-4 pt-4 justify-center md:justify-start">
                ...
                <button onClick={() => fileInputRef.current.click()} className="bg-white border border-slate-100 px-8 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-900 shadow-sm hover:bg-slate-50 transition-all">Upload File</button>
             </div>
          </div>
          ...
          <div className="relative w-full max-w-sm">
             ...
             <div className="relative glass-card p-8 rounded-[2.5rem] border border-white/40 shadow-2xl space-y-6">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      <h4 className="text-sm font-black text-slate-900 italic">Analysis Status</h4>
      ...
      <div className="grid grid-main items-start">
        ...
        <div className="col-span-4 lg:col-span-4 lg:sticky lg:top-32 space-y-8">
          <div className="glass-card p-10 rounded-[3rem] bg-purple-50/40 backdrop-blur-md border border-purple-100 shadow-glass">
            <h3 className="text-xl font-black mb-8 text-slate-900 italic">Upload Center</h3>
            <div
            ...
            >
              ...
              <div className="text-center pointer-events-none">
                ...
                <p className="text-[9px] font-black text-purple-400 uppercase tracking-widest leading-relaxed">Drop PDF here<br/>to analyze</p>
              </div>
            </div>
            ...
            {file && (
              <div className="mt-8 p-6 bg-purple-500/5 rounded-2xl border border-purple-500/10 flex items-center justify-between animate-pop">
                ...
                <button onClick={handleUpload} disabled={isLoading} className="bg-purple-500 text-white px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-purple-500/20">{isLoading ? "Wait..." : "Upload"}</button>
              </div>
            )}
          </div>
          ...
          {selectedCvsForCompare.length === 2 && (
            <div className="p-10 rounded-[3rem] bg-purple-600 text-white shadow-2xl animate-pop">
              ...
              <h4 className="text-lg font-black mb-6 italic">Compare CVs</h4>
              <button onClick={handleCompare} disabled={isComparing} className="w-full py-5 bg-white text-purple-600 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:-translate-y-1 transition-all">{isComparing ? "Analyzing..." : "Compare with AI ⚖️"}</button>
            </div>
          )}
        </div>
        ...
        <div className="col-span-4 lg:col-span-10 space-y-12">
          <div className="flex justify-between items-center px-4">
            <div>
              <h3 className="text-2xl font-black text-slate-900 italic">My Documents</h3>
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Uploaded CVs</p>
            </div>
            <span className="bg-purple-50/50 border border-purple-100 px-5 py-2 rounded-full text-[9px] font-black text-purple-500 uppercase tracking-widest shadow-sm">{cvList.length} Files</span>
          </div>
          ...
          <div className="glass-card p-12 rounded-[4rem] bg-purple-50/30 backdrop-blur-xl border border-purple-100 shadow-glass flex flex-col items-center text-center space-y-8">
             ...
             <p className="text-sm font-medium text-slate-400 max-w-xl">Find the best career path based on your resume.</p>
             <button onClick={() => window.location.href = "/career-match"} className="bg-purple-500 text-white px-12 py-5 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-purple-500/20 hover:-translate-y-1 transition-all">Find My Career Match →</button>
          </div>
        </div>
      </div>
      <UserFlowFooter currentPath="/cv-manager" />
    </div>
  );
};
